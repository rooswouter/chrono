...
double fps = 60;
const std::string out_dir = GetChronoOutputPath() + "ADVANCD_OPENCRGRIDE_DEMO";
...
int main(int argc, char* argv[])
{
    ...
    ChWheeledVehicleAdvanced vehicle(&sys, GetVehicleDataFile(vehicle_model->VehicleJSON()));
    vehicle.Initialize(init_csys);
    ...
    MyDriver driver(driver_type, vehicle, mph_to_mps * speed, path);
    driver.Initialize();
    ...
    vis->SetWindowTitle("Advanded OpenCRGRideTest");
    ...
    while (true) {
        ...
        ChVector3d xpos = vehicle.GetPos();
        ChVector3d sacc = vehicle.GetPointAcceleration(ChVector3d(-1.0, 1.0, 0.5));
        double vel = vehicle.GetSpeed();
        if (xpos.x() > 100.0) {
